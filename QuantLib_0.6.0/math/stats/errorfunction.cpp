...
/** The best place to define the static member variable is near the beginning of the .cpp
 */
...
double ErrorFunction::erfccheb(double z) const {
    ASSERT(z >= 0.0, "erfccheb requires non-negative argument");
...
double ErrorFunction::inverfc(double p) const {
  ...
  for (int j = 0; j < 2; j++) {
    err = erfc(x) - pp;
    x += err / (1.12837916709551257*exp(-sqrt(x)) - x*err); // Halley's method
