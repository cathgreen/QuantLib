...
#ifndef SOBOLURNG_HPP
#define SOBOLURNG_HPP
...
private:
  ...
  vector<double> point_; // the Sobol point in dim_ dimensions
  size_t curridx_;            // the current index in the point_ vector

  //	long*	pol;
  vector<long> otpol;  // vector with the polynomial encodings
  vector<long> deg;    // vector with the corresponding polynomial degrees
  long*	iv;                 // MAXBIT * ndim matrix of integer values
  long	in;
  vector<long>	ix;     // the vector of components
  vector<long*>	iu;   // allows 2D access into iv
...
: dim_(dimension), point_(dimension), curridx_(dimension),
  otpol(dimension), deg(dimension), in(0), ix(dimension), iu(MAXBIT), fac(1.00 / (1L << MAXBIT))
{
    ASSERT(dimension > 0, "the dimension must be positive!");
    init(dimension);
}
...
void SobolURng::next(ITER begin, ITER end)
{
  ...
  for (auto it = begin; it != end; ++it)
    ncomp++;
  ASSERT(ncomp <= dim_, "SobolURng::next(), size of range to fill is too large");
  ASSERT( dim_ % ncomp == 0, "SobolURng::next(), size of range to fill is not a divisor of dim")
