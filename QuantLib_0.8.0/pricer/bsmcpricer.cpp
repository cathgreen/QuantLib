      ...
      mcparams_(mcparams)
{
    ...
    if (mcparams.pathGenType == McParams::PathGenType::EULER) {
        if (mcparams.urngType == McParams::UrngType::MINSTDRAND)
            pathgen_ = SPtrPathGenerator(new EulerPathGenerator<NormalRngMinStdRand>(fixtimes.begin(), fixtimes.end(), 1));
        else if (mcparams.urngType == McParams::UrngType::MT19937)
            pathgen_ = SPtrPathGenerator(new EulerPathGenerator<NormalRngMt19937>(fixtimes.begin(), fixtimes.end(), 1));
        else if (mcparams.urngType == McParams::UrngType::RANLUX3)
            pathgen_ = SPtrPathGenerator(new EulerPathGenerator<NormalRngRanLux3>(fixtimes.begin(), fixtimes.end(), 1));
        else if (mcparams.urngType == McParams::UrngType::RANLUX4)
            pathgen_ = SPtrPathGenerator(new EulerPathGenerator<NormalRngRanLux4>(fixtimes.begin(), fixtimes.end(), 1));
        else if (mcparams.urngType == McParams::UrngType::SOBOL)
            pathgen_ = SPtrPathGenerator(new EulerPathGenerator<NormalRngSobol>(fixtimes.begin(), fixtimes.end(), 1));
        else
